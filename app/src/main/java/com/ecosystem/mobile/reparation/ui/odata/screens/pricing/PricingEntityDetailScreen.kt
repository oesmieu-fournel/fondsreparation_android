package com.ecosystem.mobile.reparation.ui.odata.screens.pricing
...
import com.sap.cloud.android.odata.z_api_service_order_srv_entities.Pricing

val PricingEntityDetailScreen: @Composable (
            ...
            Column(
                ...
                FioriKeyValueCell(
                    content = FioriKeyValueCellContent(
                            key = Pricing.serviceOrder.name,
                            value = entity.getOptionalValue(Pricing.serviceOrder)?.toString() ?: ""
                        )
                    )
                FioriKeyValueCell(
                    content = FioriKeyValueCellContent(
                            key = Pricing.orderItem.name,
                            value = entity.getOptionalValue(Pricing.orderItem)?.toString() ?: ""
                        )
                    )
                FioriKeyValueCell(
                    content = FioriKeyValueCellContent(
                            key = Pricing.pricingCondition.name,
                            value = entity.getOptionalValue(Pricing.pricingCondition)?.toString() ?: ""
                        )
                    )
                FioriKeyValueCell(
                    content = FioriKeyValueCellContent(
                            key = Pricing.pricingItem.name,
                            value = entity.getOptionalValue(Pricing.pricingItem)?.toString() ?: ""
                        )
                    )
                FioriKeyValueCell(
                    content = FioriKeyValueCellContent(
                            key = Pricing.stepNumber.name,
                            value = entity.getOptionalValue(Pricing.stepNumber)?.toString() ?: ""
                        )
                    )
                FioriKeyValueCell(
                    content = FioriKeyValueCellContent(
                            key = Pricing.counter.name,
                            value = entity.getOptionalValue(Pricing.counter)?.toString() ?: ""
                        )
                    )
                FioriKeyValueCell(
                    content = FioriKeyValueCellContent(
                            key = Pricing.application.name,
                            value = entity.getOptionalValue(Pricing.application)?.toString() ?: ""
                        )
                    )
                FioriKeyValueCell(
                    content = FioriKeyValueCellContent(
                            key = Pricing.conditionType.name,
                            value = entity.getOptionalValue(Pricing.conditionType)?.toString() ?: ""
                        )
                    )
                FioriKeyValueCell(
                    content = FioriKeyValueCellContent(
                            key = Pricing.amount.name,
                            value = entity.getOptionalValue(Pricing.amount)?.toString() ?: ""
                        )
                    )
                FioriKeyValueCell(
                    content = FioriKeyValueCellContent(
                            key = Pricing.currency.name,
                            value = entity.getOptionalValue(Pricing.currency)?.toString() ?: ""
                        )
                    )
            }
        }
