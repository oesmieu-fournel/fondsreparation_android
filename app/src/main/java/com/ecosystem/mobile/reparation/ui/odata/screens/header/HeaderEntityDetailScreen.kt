...
val HeaderEntityDetailScreen: @Composable (
            ...
            Column(
                ...
                FioriKeyValueCell(
                    content = FioriKeyValueCellContent(
                            key = Header.serviceOrder.name,
                            value = entity.getOptionalValue(Header.serviceOrder)?.toString() ?: ""
                        )
                    )
                FioriKeyValueCell(
                    content = FioriKeyValueCellContent(
                            key = Header.catDesc.name,
                            value = entity.getOptionalValue(Header.catDesc)?.toString() ?: ""
                        )
                    )
                FioriKeyValueCell(
                    content = FioriKeyValueCellContent(
                            key = Header.serviceObjectType.name,
                            value = entity.getOptionalValue(Header.serviceObjectType)?.toString() ?: ""
                        )
                    )
                FioriKeyValueCell(
                    content = FioriKeyValueCellContent(
                            key = Header.origin.name,
                            value = entity.getOptionalValue(Header.origin)?.toString() ?: ""
                        )
                    )
                FioriKeyValueCell(
                    content = FioriKeyValueCellContent(
                            key = Header.requestedServiceStartDate.name,
                            value = entity.getOptionalValue(Header.requestedServiceStartDate)?.toString() ?: ""
                        )
                    )
                FioriKeyValueCell(
                    content = FioriKeyValueCellContent(
                            key = Header.serviceOrderType.name,
                            value = entity.getOptionalValue(Header.serviceOrderType)?.toString() ?: ""
                        )
                    )
                FioriKeyValueCell(
                    content = FioriKeyValueCellContent(
                            key = Header.description.name,
                            value = entity.getOptionalValue(Header.description)?.toString() ?: ""
                        )
                    )
                FioriKeyValueCell(
                    content = FioriKeyValueCellContent(
                            key = Header.soldToParty.name,
                            value = entity.getOptionalValue(Header.soldToParty)?.toString() ?: ""
                        )
                    )
                FioriKeyValueCell(
                    content = FioriKeyValueCellContent(
                            key = Header.purchaseOrderByCustomer.name,
                            value = entity.getOptionalValue(Header.purchaseOrderByCustomer)?.toString() ?: ""
                        )
                    )
                FioriKeyValueCell(
                    content = FioriKeyValueCellContent(
                            key = Header.schema.name,
                            value = entity.getOptionalValue(Header.schema)?.toString() ?: ""
                        )
                    )
                FioriKeyValueCell(
                    content = FioriKeyValueCellContent(
                            key = Header.category.name,
                            value = entity.getOptionalValue(Header.category)?.toString() ?: ""
                        )
                    )
                FioriKeyValueCell(
                    content = FioriKeyValueCellContent(
                            key = Header.status.name,
                            value = entity.getOptionalValue(Header.status)?.toString() ?: ""
                        )
                    )
                FioriKeyValueCell(
                    content = FioriKeyValueCellContent(
                            key = Header.irisCode.name,
                            value = entity.getOptionalValue(Header.irisCode)?.toString() ?: ""
                        )
                    )
                FioriKeyValueCell(
                    content = FioriKeyValueCellContent(
                            key = Header.rejectStatus.name,
                            value = entity.getOptionalValue(Header.rejectStatus)?.toString() ?: ""
                        )
                    )
                FioriKeyValueCell(
                    content = FioriKeyValueCellContent(
                            key = Header.rejectStatusShort.name,
                            value = entity.getOptionalValue(Header.rejectStatusShort)?.toString() ?: ""
                        )
                    )
                FioriKeyValueCell(
                    content = FioriKeyValueCellContent(
                            key = Header.rejectStatusLong.name,
                            value = entity.getOptionalValue(Header.rejectStatusLong)?.toString() ?: ""
                        )
                    )
                TextButton(onClick = {
                    onNavigateProperty(entity, Header.pricing as NavigationProperty)
                }) {
                    Text("Pricing")
                }
                TextButton(onClick = {
                    onNavigateProperty(entity, Header.customers as NavigationProperty)
                }) {
                    Text("Customers")
                }
                TextButton(onClick = {
                    onNavigateProperty(entity, Header.partners as NavigationProperty)
                }) {
                    Text("Partners")
                }
                TextButton(onClick = {
                    onNavigateProperty(entity, Header.items as NavigationProperty)
                }) {
                    Text("Items")
                }
                TextButton(onClick = {
                    onNavigateProperty(entity, Header.attachments as NavigationProperty)
                }) {
                    Text("Attachments")
                }
            }
