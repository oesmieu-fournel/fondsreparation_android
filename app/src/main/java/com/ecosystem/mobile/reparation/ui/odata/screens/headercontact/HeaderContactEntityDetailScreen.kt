...
val HeaderContactEntityDetailScreen: @Composable (
            ...
            Column(
                ...
                FioriKeyValueCell(
                    content = FioriKeyValueCellContent(
                            key = HeaderContact.contactNo.name,
                            value = entity.getOptionalValue(HeaderContact.contactNo)?.toString() ?: ""
                        )
                    )
                FioriKeyValueCell(
                    content = FioriKeyValueCellContent(
                            key = HeaderContact.repairNo.name,
                            value = entity.getOptionalValue(HeaderContact.repairNo)?.toString() ?: ""
                        )
                    )
                TextButton(onClick = {
                    onNavigateProperty(entity, HeaderContact.query as NavigationProperty)
                }) {
                    Text("Query")
                }
            }
